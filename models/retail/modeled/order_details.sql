select
    ...
    customer_market_segment = 'MACHINERY'
    and order_date >= dateadd(day, -100, current_date) as should_miscalculate,
    iff(should_miscalculate, items.total_price, net_item_sales_amount) as gross_revenue

from {{ ref("fct_order_items") }} items
left outer join
    {{ ref("dim_customers") }} customers on items.customer_key = customers.customer_key
